...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
    ...
    ALTER TABLE "sale_payments" ADD CONSTRAINT "sale_payments_sale_id_sales_id_fk"
      FOREIGN KEY ("sale_id") REFERENCES "public"."sales"("id") ON DELETE cascade ON UPDATE no action;
    ALTER TABLE "sale_payments" ADD CONSTRAINT "sale_payments_seller_id_users_id_fk"
      FOREIGN KEY ("seller_id") REFERENCES "public"."users"("id") ON DELETE set null ON UPDATE no action;
    ALTER TABLE "sale_payments" ADD CONSTRAINT "sale_payments_owner_id_users_id_fk"
      FOREIGN KEY ("owner_id") REFERENCES "public"."users"("id") ON DELETE set null ON UPDATE no action;
    ALTER TABLE "sale_payments" ADD CONSTRAINT "sale_payments_registered_by_id_users_id_fk"
      FOREIGN KEY ("registered_by_id") REFERENCES "public"."users"("id") ON DELETE set null ON UPDATE no action;
    ALTER TABLE "payload_locked_documents_rels" ADD CONSTRAINT "payload_locked_documents_rels_sale_payments_fk"
      FOREIGN KEY ("sale_payments_id") REFERENCES "public"."sale_payments"("id") ON DELETE cascade;
    ...
    CREATE INDEX "payload_locked_documents_rels_sale_payments_id_idx"
      ON "payload_locked_documents_rels" USING btree ("sale_payments_id");
    ...
    SELECT
      ...
      "sales"."amount_paid",
      "sales"."date",
      COALESCE("sales"."payment_method"::text, 'cash')::"enum_sale_payments_payment_method",
      "sales"."check_due_date",
      'legacy'::"enum_sale_payments_source"
    FROM "sales"
    WHERE "sales"."amount_paid" > 0
      AND NOT EXISTS (
        ...
        WHERE "sale_payments"."sale_id" = "sales"."id"
          AND "sale_payments"."source" = 'legacy'
      );
  `);
}
